...
function AgentNode({ data }: NodeProps<AgentNodeData>) {
  ...
  const FrameworkIcon = frameworkDetails[data.framework as keyof typeof frameworkDetails]?.icon || Bot;
  const iconColor = frameworkDetails[data.framework as keyof typeof frameworkDetails]?.color || 'text-purple-400';
  ...
  return (
    <div className="p-4 rounded-xl border-2 border-neutral-700 bg-neutral-900 text-white w-72 shadow-xl hover:border-purple-500 transition-colors">
      ...
      <div className="text-xs text-white/70 bg-neutral-800 px-2 py-1 rounded-full inline-block">
        Framework: <span className="font-semibold">{data.framework}</span>
      </div>
